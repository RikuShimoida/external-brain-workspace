...
worktree が隔離するのは Git 管理下のファイルだけ（§2）。逆に言えば、**worktree の中で新しく作った
Git 管理外のファイル（リンクでない実ファイル）は、撤去と一緒に消える**。

- 実例: #55 で機械式NG の語彙 `scripts/ng_words.txt`（センシティブなので `.gitignore` 済み）を worktree 内で作った。
  `.worktreeinclude` に載っていなかったため撤去で消え、読み込み側が「無ければ空」で黙って動いたので
  2週間気づかれなかった（#70 で Claude Code のログから復元）。
- 対策1: 残すべき Git 管理外ファイルは**本体側に置き**、`.worktreeinclude` に `link <パス>` を足す（ファイル単位でも可）。
- 対策2: `/impl` の 2-3 で、撤去の前に「リンクでない Git 管理外ファイル」が残っていないかを確認する。
- 対策3: 必須ファイルを読むコードは、無いときに**黙らず警告を出す**（`load_ng_patterns()`）。
