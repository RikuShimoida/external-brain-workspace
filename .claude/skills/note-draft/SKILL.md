...
- **提案の前に必ず既出をチェックする**（手順 0.5）。同じ角度での焼き直しは事故として扱う。
...
| 状況 | 扱い |
|---|---|
| `status` が `posted`（X に投稿済み） | **除外する** |
| それ以外の既出（下書き止まり／別スキルの生成物） | **除外しない。別角度で出す** |

**除外しないのが原則。** 素材は有限（候補 1,051 件）で、完全除外だと早晩枯れる。
むしろ note は「Podcast で話したことを記事にする」形で**意図的に同じ素材を跨ぐ**媒体なので、
`podcast-ideas/` と重なること自体は問題ない。問題なのは**前回と同じ角度で書くこと**。
再利用するときは `既出` 欄に前回どう扱ったかを明記し、今回の切り口の違いを書く。
...
- **本人の口調に寄せる。** 書く前に `voice-archive/speech_profile.md`（しゃべるときの話し方プロファイル）の
  「6. 台本に寄せるときのルール」を読む。note はそのまま Podcast の骨組みになるので、書き言葉に寄りすぎない。
  ファイルが無ければ飛ばしてよい。
