#!/usr/bin/env python3
"""Facebook Messenger のトーク履歴から「濃い日」だけ本文を抽出する。

その日いちばん長いメッセージが閾値を超える日だけを Markdown 化する。
長文が数件ある日は相談・思考整理、短文が大量の日は雑談。文字数の合計ではなく
「最長メッセージ」で見ると前者だけを拾える。LINE の line_extract.py と同じ考え方。

抜き出すのはその日のやりとり全部。長文だけ切り出すと前後の文脈が消えるため。

プライバシー: 相手の実名は成果物に出さない方針のため、出力ファイル名・見出しには
スレッド ID（<ローマ字名>_<数字>）を使い、タイトル（実名を含みうる）は書かない。

使い方:
    python3 scripts/messenger_extract.py [閾値] [--dry-run] [--room ID]
      閾値      … その日の最長メッセージの文字数の下限（既定 300）
      --dry-run … 書き出さず対象日だけ表示
      --room    … スレッド ID で絞る（部分一致）

出力:
    messenger-archive/deep/YYYY-MM-DD_スレッドID.md
"""
import os
import re
import sys
import glob
import datetime as dt
from collections import defaultdict

from messenger_parse import parse, thread_id

OUTDIR = "messenger-archive/deep"
THRESHOLD = 300
GLOBS = [
    "messenger-archive/_raw/your_facebook_activity/messages/inbox/*/message_*.json",
    "messenger-archive/_raw/your_facebook_activity/messages/message_requests/*/message_*.json",
    "messenger-archive/_raw/your_facebook_activity/messages/archived_threads/*/message_*.json",
    "messenger-archive/_raw/your_facebook_activity/messages/filtered_threads/*/message_*.json",
]


def safe_name(s):
    """ファイル名に使える形に均す。"""
    s = re.sub(r"[^\w぀-ヿ一-鿿 ー-]", "", s)
    return s.strip().replace(" ", "_")[:40] or "無題"


def _dt(ts_ms):
    return dt.datetime.fromtimestamp(ts_ms / 1000)


def main():
    argv = sys.argv[1:]
    dry = "--dry-run" in argv
    if dry:
        argv.remove("--dry-run")
    room_filter = None
    if "--room" in argv:
        i = argv.index("--room")
        room_filter = argv[i + 1]
        del argv[i : i + 2]
    nums = [a for a in argv if not a.startswith("--")]
    threshold = int(nums[0]) if nums else THRESHOLD

    paths = []
    for g in GLOBS:
        paths.extend(glob.glob(g))
    paths = sorted(paths)
    if not paths:
        sys.exit("メッセージが見つかりません: messenger-archive を展開しましたか？")

    # (日付, スレッドID) -> その日のメッセージ
    days = defaultdict(list)
    for path in paths:
        _title, messages = parse(path)
        tid = thread_id(path)
        if room_filter and room_filter not in tid:
            continue
        for msg in messages:
            date = _dt(msg.ts_ms).strftime("%Y/%m/%d") if msg.ts_ms else ""
            days[(date, tid)].append(msg)

    picked = []
    for (date, room), msgs in days.items():
        longest = max((len(m.body) for m in msgs if not m.is_attachment), default=0)
        if longest >= threshold:
            picked.append((date, room, longest, msgs))
    picked.sort(key=lambda x: (x[0], x[1]))

    total_chars = sum(sum(len(m.body) for m in p[3]) for p in picked)
    print(f"最長 {threshold} 字以上の日: {len(picked)} 日（本文計 {total_chars:,} 字）")

    if dry:
        for date, room, longest, msgs in picked:
            print(f"  {date}  {room[:28]:<28}  {len(msgs):>3}件  最長{longest:>5}字")
        return

    os.makedirs(OUTDIR, exist_ok=True)
    for date, room, longest, msgs in picked:
        fname = f"{OUTDIR}/{date.replace('/', '-')}_{safe_name(room)}.md"
        with open(fname, "w", encoding="utf-8") as w:
            w.write(f"# {room} {date}\n\n")
            w.write(f"- メッセージ数: {len(msgs)}\n- 最長: {longest} 字\n\n---\n\n")
            for m in msgs:
                t = _dt(m.ts_ms).strftime("%H:%M") if m.ts_ms else "--:--"
                body = m.body if m.body else f"（添付 {m.attachments} 件）"
                w.write(f"**{t} {m.sender}:**\n\n{body}\n\n")
    print(f"{len(picked)} 件を {OUTDIR}/ に書き出しました。")


if __name__ == "__main__":
    main()
