...
- **freee は読むのは自由、書くのは `/freee` の自動登録ルール作成だけ。** freee MCP は `freee_api_get` と状態確認系だけ（`freee_api_post/put/patch/delete` は使わない）。
...
- **Google 検索履歴はセンシティブ検索を含む。** アダルト・極私的な健康/金銭などを成果物に出さない。
  ...
  **YouTube も同じ二段フィルタ**（判定単位はチャンネル名・再生リスト名 → `youtube-archive/verdicts.tsv`）。
...
7. **壁打ちの結論は「日記」へ残す（記載前に必ず確認）。** 相談・議論・思考整理・意思決定・アイデア出しをして、あとで見返す価値のある結論・気づき・決定・ネタが出たら、そのやり取りの締めで **「日記に記載しますか？」とオーナーに質問**し、承認を得てから、その日の「今日やること - @YYYY-MM-DD」ノートの **日記** 欄（旧名: なんでもメモ）へ要約を記載する。**質問せずに勝手に書き込まない。** 迷ったら（残す価値があるか判断に迷うくらいなら）オファー側に倒す。手順は [braindump-to-memo スキル](.claude/skills/braindump-to-memo/SKILL.md)。
8. **枠の中で比べる前に、枠そのものを一度疑う。** 相談・計画・意思決定・実装の依頼で細かい比較に入る前に、「そもそもこの前提・起点・目的で合っているか」を一度だけ問う。当たり前に見える制約ほど疑う。ただし予約済み・決定済みのことは蒸し返さない。
